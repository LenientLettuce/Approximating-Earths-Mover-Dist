...
def approximate_emd_optimized(C, max_iterations=5000, gamma=0.05):
    """
    Highly optimized implementation of the Beretta & Rubinstein EMD Approximation.
    - Vectorizes the slack matrix calculation.
    - Replaces Python sets with NumPy boolean masks for O(1) C-level lookups.
    - Uses Hopcroft-Karp dead-end pruning to prevent DFS exponential blowup.
    """
    n = C.shape[0]
    ...
    # Pre-allocate structures to avoid memory overhead in loops
    dist = np.empty(n, dtype=np.float64)
    queue = collections.deque()
    
    for iteration in range(max_iterations):
        # ---------------------------------------------------------
        # OPTIMIZATION 1: Compute Slack and Eligibility ONCE
        # ---------------------------------------------------------
        # Broadcasting handles C (n, n) - phi_u (n, 1) - phi_v (1, n)
        slack = C - phi_u[:, np.newaxis] - phi_v[np.newaxis, :]
        eligible_edges = slack <= 1e-8
        
        # ---------------------------------------------------------
        # STEP 1: Node-Disjoint Augmenting Paths (Hopcroft-Karp)
        # ---------------------------------------------------------
        dist.fill(np.inf)
        queue.clear()
        ...
        # A. BFS Layering (Vectorized neighbor lookups)
        while queue:
            u = queue.popleft()
            if dist[u] < dist_dummy:
        ...
        # B. DFS Path Extraction with Dead-End Pruning
        # OPTIMIZATION 2: Global visited array stops O(E * sqrt(V)) exponential blowups
        ...
        def dfs(u):
            ...
            for v in eligible_v:
                ...
                visited_v_global[v] = True  # Mark immediately
        ...
        # If we successfully made matches, skip the dual update this round 
        # and immediately re-run BFS/DFS. This avoids heavy tree building.
        if matches_added > 0:
            continue
        ...
        # ---------------------------------------------------------
        # STEP 2: Reachability Forest & Dual Update
        # ---------------------------------------------------------
        # OPTIMIZATION 3: Boolean masks instead of Python sets
        ...
        while queue_u:
            ...
            # Fast filter for unvisited columns
            new_v = eligible_v[~in_F1[eligible_v]]
            in_F1[new_v] = True
            
            # Fast lookup of matching rows
            u_nexts = mate_v[new_v]
            
            # Filter rows: must be matched (not -1) and not already in F0
            valid_u_nexts = u_nexts[(u_nexts != -1) & (~in_F0[u_nexts])]
            in_F0[valid_u_nexts] = True
            queue_u.extend(valid_u_nexts)
            
        # D. The Delta-Jump (Highly Vectorized)
        if not in_F0.any() or in_F1.all():
            break 
            
        # OPTIMIZATION 4: Slice the already-calculated slack matrix
        # in_F0 extracts the reachable rows, ~in_F1 extracts the unreachable columns
    ...
    # The actual percentage of nodes we left unmatched
    gamma_actual = num_unmatched / n 

    if num_unmatched == 0:
        return matched_cost, mate_u

    #Basically Making sure estimated cost scales inversely with range.
    range2 = np.log10(10000)
    estimated_remaining_cost = (avg_matched_cost) * (range2/n +  (range2 / np.log2(n)) * n + n / range2)
...
if __name__ == "__main__":
    ...
    cost, matching = approximate_emd_optimized(C_float, gamma=0.05)
